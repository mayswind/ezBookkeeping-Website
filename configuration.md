...
| `map_data_fetch_proxy` | `false` | Whether use the ezbookkeeping server to forward map data requests for `openstreetmap`, `openstreetmap_humanitarian`, `opentopomap`, `opnvkarte`, `cyclosm`, `cartodb`, `tomtom`, `tianditu` or `custom`. |
...
| `tianditu_map_app_key` |  | TianDiTu map application key for `tianditu`, please visit [https://console.tianditu.gov.cn/api/register](https://console.tianditu.gov.cn/api/register) for more information. |
