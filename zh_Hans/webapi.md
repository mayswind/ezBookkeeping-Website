...
| max_time | int64 | 可选 | 最大时间序列Id（若获取最新内容设置为 `0`） |
| min_time | int64 | 可选 | 最小时间序列Id |
| page | int32 | 可选 | 指定页码 |
| count | int32 | 必选 | 每页交易的数量（最大为 `50`） |
| with_count | bool | 可选 | 是否获取所有交易总数量 |
| with_pictures | bool | 可选 | 是否获取图片IDs |
| trim_account | bool | 可选 | 是否获取账户ID而不是账户对象 |
| trim_category | bool | 可选 | 是否获取交易分类ID而不是交易分类对象 |
| trim_tag | bool | 可选 | 是否获取交易标签ID而不是交易标签对象 |
...
| nextTimeSequenceId | int64 | 请求更旧数据时的下一个游标（`max_time` 参数） |
| totalCount | int64 | 交易总数量 |
