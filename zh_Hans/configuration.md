...
| `domain` | `localhost` | 访问 ezBookkeeping 使用的域名（例如 `ezbookkeeping.mayswind.net`）。 |
| `root_url` | `%(protocol)s://%(domain)s:%(http_port)s/` | 在浏览器中访问 ezBookkeeping 的完整路径（例如 `https://ezbookkeeping.mayswind.net/`）。如果用户在浏览器访问时的协议和端口号与上述设置（`protocol` 和 `http_port`）相同，该项可以使用默认值。 |
