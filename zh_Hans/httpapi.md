...
ezBookkeeping 的 API 通过令牌来验证用户身份，您可以修改配置文件中的 `enable_generate_api_token` 或修改环境变量开启生成 API 令牌的功能，具体可以参考 [配置](/zh_Hans/configuration#安全)。开启后在桌面版的 “用户设置” -> “安全” 页面中会出现 “生成令牌” 按钮，也可以通过 ezBookkeeping 的命令行工具 `user-session-new` 进行生成，具体可以参考 [命令行](/zh_Hans/command_line#userdata)。
