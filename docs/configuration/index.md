...
| `openai_compatible_base_url` |  | OpenAI compatible API base url for `openai_compatible` (Use Chat Completions API) and `openai_responses_compatible` (Use Responses API) LLM provider, e.g. `https://api.openai.com/v1/`. |
| `openai_compatible_api_key` |  | OpenAI compatible API Key for `openai_compatible` (Use Chat Completions API) and `openai_responses_compatible` (Use Responses API) LLM provider. |
...
| `ollama_model_id` |  | Model ID for the `ollama` LLM provider, used for transaction text recognition and transaction creation. |
