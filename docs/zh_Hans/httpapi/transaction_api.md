...
| `sourceAmount` | `integer` | 来源金额（最多支持两位小数，例如该字段为 `1234` 时表示金额为 `12.34`） |
| `destinationAmount` | `integer` | 目标金额（最多支持两位小数，例如该字段为 `1234` 时表示金额为 `12.34`） |
...
| `match_mode` | `integer` | 可选 | 关键词匹配模式（`0`：数据库默认设置，`1`：忽略大小写） |
