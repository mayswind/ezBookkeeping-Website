...
## 如何记录商户、付款人、收款人、项目等信息

ezBookkeeping 采用交易标签来记录商户、付款人、收款人、项目等信息。你可以为每笔交易添加一个或多个标签来记录这些信息。同时，ezBookkeeping 支持标签组，你可以创建不同的标签组来区分不同类型的标签。

## 如何使用最后对账时间

ezBookkeeping 允许用户在用户个人信息设置中开启 “使用最后对账时间” 功能，开启后你可以在账户编辑页面更新或取消最后对账时间，也可以在对账单页面中将账户最后对账时间更新为对账单的结束时间，或在账户列表界面将对最后对账时间更新为当前时间。

你可以搭配 “可编辑交易范围” 功能使用最后对账时间，例如将 “可编辑交易范围” 设置为 “最后对账时间或更晚”，这样最后对账时间之前的交易将无法被修改。

## 为什么添加新交易时提示 “您不能添加该交易时间的交易”

你如果在用户信息中设置了 “可编辑交易范围”，那么你不能添加、导入、编辑或删除在这个时间范围的交易。你可以随时在用户信息界面修改这个设置。
...
进入用户信息修改页面，即可修改创建新交易时默认使用的账户。

## 如何更改金额的颜色

进入用户信息修改页面，即可更改支出与收入金额的颜色。

## 移动版交易列表中如何筛选标签

在移动版中，轻触交易列表页面右下角的三个点，在弹出的菜单中即可筛选标签。
...
1. 在移动版中，长按主界面底部导航栏的创建交易按钮，轻触弹出列表中的 “AI剪贴板文本识别” 即可。或进入创建交易页面，轻触右上角的更多按钮，在弹出的菜单中选择 “AI剪贴板文本识别” 即可
2. 在桌面版中，将鼠标移动到交易列表页的添加交易按钮，点击下拉列表中的 “AI剪贴板文本识别” 即可。或进入创建交易对话框，轻触右上角的 “AI剪贴板文本识别” 按钮即可
...
4. 大语言模型-AI剪贴板文本识别 / AI识图 / 导入 AI 文本内容识别结果：当用户使用 ezBookkeeping 的这些功能时，ezBookkeeping 会将用户粘贴的文本或上传的收据图片以及用户所有的交易分类名称、账户名称和交易标签名称数据发送给大语言模型提供者。ezBookkeeping 默认不启用这些功能，同时 ezBookkeeping 建议使用自托管的大语言模型服务。如果你使用第三方的大语言模型服务，请注意潜在的隐私风险，并采取措施减少数据的泄露（例如，粘贴前去除文本中的敏感信息、上传收据图片前去除敏感信息，以及避免在交易分类名、账户名、交易标签名中存储敏感信息等）
