...
ezBookkeeping 支持导入使用分隔符分隔的自定义格式数据，您可以将您要导入的数据使用逗号、制表符或分号分隔，然后将其保存为 CSV、TSV 或 SSV 格式的文件，或者直接将其粘贴到文本框中，也可以保存为 Excel 工作簿文件。

1. 在 “交易详情” 页面中点击 “导入” 按钮
2. 选择 “分隔符分隔值 (DSV) 文件”、“分隔符分隔值 (DSV) 数据” 或 “Excel 工作簿文件” 作为文件类型
    1. 对于 “分隔符分隔值 (DSV) 文件”，需要选择文件编码并选择导入的文件
    2. 对于 “分隔符分隔值 (DSV) 数据”，直接在下方文本框粘贴文件的内容
    3. 对于 “Excel 工作簿文件”，需要选择导入的文件
    4. 然后，对于 “分隔符分隔值 (DSV) 文件” 或 “分隔符分隔值 (DSV) 数据”，需要选择格式为 “CSV (逗号分隔的值) 文件”、“TSV (制表符分隔的值) 文件” 或 “SSV (分号分隔的值) 文件”。对于 “Excel 工作簿文件”，需要选择格式为 “Excel 工作簿文件 (.xlsx)” 或 “Excel 97-2003 工作簿文件 (.xls)”
    5. 然后根据需要选择处理方法为 “列映射” 或 “自定义脚本”，点击 “下一步” 按钮
...
> [!NOTE] 提示
> 随手记 iPhone App 导出的交易数据中没有正确处理交易备注中的换行符，如果交易备注中包含换行符，导入 ezbookkeeping 将会失败。如果存在该问题，可以使用随手记 Android App 导出交易数据，然后再导入到 ezbookkeeping。
...
> [!NOTE] 提示
> 随手记（Web版）导出的转账交易数据中没有正确处理跨币种的转账交易数据，跨币种的转账交易只有一个金额。如果您要导入的数据中包含跨币种的转账交易，请使用随手记手机App导出交易数据，然后再导入到 ezbookkeeping。
