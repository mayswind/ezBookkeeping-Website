...
| 选项名 | 默认值 | 描述 |
| --- | --- | --- |
| `type` | `local_filesystem` | 对象存储类型，当前支持 `local_filesystem`、`s3`、`minio` 和 `webdav`。 |
| `local_filesystem_path` | `storage/` | 使用 `local_filesystem` 对象存储时，对象文件的根目录。该值可以设置为相对或绝对路径。 |
| `s3_endpoint` | `127.0.0.1:9000` | 使用 `s3` 对象存储时，S3 兼容服务的端点地址。 |
| `s3_region` | `us-east-1` | 使用 `s3` 对象存储时，S3 兼容服务的区域，该选项必须设置，若自建服务无区域信息，可任意设置，如 `us-east-1`。 |
| `s3_access_key_id` |  | 使用 `s3` 对象存储时，S3 兼容服务的 Access Key ID。 |
| `s3_secret_access_key` |  | 使用 `s3` 对象存储时，S3 兼容服务的 Secret Access Key。 |
| `s3_session_token` |  | 使用 `s3` 对象存储时，S3 兼容服务的会话令牌。 |
| `s3_use_ssl` | `false` | 使用 `s3` 对象存储时，连接是否启用 SSL。 |
| `s3_skip_tls_verify` | `false` | 使用 `s3` 对象存储时，是否校验 S3 兼容服务器的证书链和主机名称。 |
| `s3_use_path_style` | `false` | 使用 `s3` 对象存储时，是否使用路径样式寻址。 |
| `s3_bucket` | `ezbookkeeping` | 使用 `s3` 对象存储时，S3 兼容服务的 Bucket 名称。 |
| `s3_root_path` | `/` | 使用 `s3` 对象存储时，Bucket 中存储对象文件的根目录。 |
...
| 选项名 | 默认值 | 描述 |
| --- | --- | --- |
| `llm_provider` |  | 用于交易文本识别的大语言模型 (LLM) 提供方，支持 `openai`、`openai_compatible`、`openai_responses_compatible`、`anthropic`、`anthropic_compatible`、`openrouter`、`ollama`、`lm_studio` 和 `google_ai`。使用第三方大语言模型提供方时，请注意潜在的隐私风险，具体参考 [常见问题 - ezBookkeeping 使用了哪些第三方服务，以及是否会使用我的个人数据](/zh_Hans/faq/#ezbookkeeping-使用了哪些第三方服务-以及是否会使用我的个人数据)。 |
| `enable_thinking` |  | 是否启用大语言模型的思考功能，支持如下级别<br/>`off`：禁用思考（支持所有提供方）。<br/>`low` / `medium` / `high` / `xhigh`：启用相应思考级别（支持`openai`、`openai_compatible`、`openai_responses_compatible`、`openrouter`、`ollama`、`lm_studio` 和 `google_ai` 提供方）。<br/>`on`：启用默认思考级别（支持 `anthropic`、`anthropic_compatible`、`ollama` 和 `lm_studio` 提供方）。<br/>留空使用大语言模型提供方默认设置。 |
...
| `openai_compatible_base_url` |  | 使用 `openai_compatible`（使用 Chat Completions API） 和 `openai_responses_compatible`（使用 Responses API） 大语言模型提供方时，OpenAI 兼容 API 的基础地址，例如 `https://api.openai.com/v1/`。 |
| `openai_compatible_api_key` |  | 使用 `openai_compatible`（使用 Chat Completions API） 和 `openai_responses_compatible`（使用 Responses API） 大语言模型提供方时，OpenAI 兼容 API 的 API Key。 |
...
| 选项名 | 默认值 | 描述 |
| --- | --- | --- |
| `llm_provider` |  | 用于收据图片识别的大语言模型 (LLM) 提供方，支持 `openai`、`openai_compatible`、`openai_responses_compatible`、`anthropic`、`anthropic_compatible`、`openrouter`、`ollama`、`lm_studio` 和 `google_ai`。使用的大语言模型提供方以及模型必须支持视觉语言模型（VLM）能力。使用第三方大语言模型提供方时，请注意潜在的隐私风险，具体参考 [常见问题 - ezBookkeeping 使用了哪些第三方服务，以及是否会使用我的个人数据](/zh_Hans/faq/#ezbookkeeping-使用了哪些第三方服务-以及是否会使用我的个人数据)。 |
| `enable_thinking` |  | 是否启用大语言模型的思考功能，支持如下级别<br/>`off`：禁用思考（支持所有提供方）。<br/>`low` / `medium` / `high` / `xhigh`：启用相应思考级别（支持`openai`、`openai_compatible`、`openai_responses_compatible`、`openrouter`、`ollama`、`lm_studio` 和 `google_ai` 提供方）。<br/>`on`：启用默认思考级别（支持 `anthropic`、`anthropic_compatible`、`ollama` 和 `lm_studio` 提供方）。<br/>留空使用大语言模型提供方默认设置。 |
| `openai_api_key` |  | 使用 `openai` 大语言模型提供方时，OpenAI 的 API Secret Key，请访问 [https://platform.openai.com/api-keys](https://platform.openai.com/api-keys) 获取更多信息。 |
