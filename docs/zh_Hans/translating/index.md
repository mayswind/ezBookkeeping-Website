...
| `longDateFormat` | [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) 中的 `LongDateFormat` | 默认长日期格式，支持 `YearMonthDay`（年月日）、`MonthDayYear`（月日年） 和 `DayMonthYear`（日月年） |
| `shortDateFormat` | [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) 中的 `ShortDateFormat` | 默认短日期格式，支持 `YearMonthDay`（年月日）、`MonthDayYear`（月日年） 和 `DayMonthYear`（日月年） |
| `longTimeFormat` | [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) 中的 `LongTimeFormat` | 默认长时间格式，支持 `HourMinuteSecond`（24小时制）、`MeridiemIndicatorHourMinuteSecond`（12小时制，上午/下午标识在前） 和 `HourMinuteSecondMeridiemIndicator`（12小时制，上午/下午标识在后） |
| `shortTimeFormat` | [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) 中的 `ShortTimeFormat` | 默认短时间格式，支持 `HourMinute`（24小时制）、`MeridiemIndicatorHourMinute`（12小时制，上午/下午标识在前） 和 `HourMinuteMeridiemIndicator`（12小时制，上午/下午标识在后） |
