...
## How to create a transaction using AI clipboard text recognition

First, you need to configure the large language model (LLM) settings. In the `llm` section, enable `transaction_from_ai_text_recognition` to allow creating transactions via AI clipboard text recognition. Then, in the `llm_text_recognition` section, set the `llm_provider`, along with the corresponding provider's API key, model ID, and other required settings. Make sure the specified model supports text recognition. For details, see [Configuration - Large Language Model](/configuration/#large-language-model).
...
## How to modify prompts of AI text recognition / AI image recognition and other LLM (Large Language Model) requests
...
2. Avatar: When using `gravatar` as the avatar provider, user's browser sends the MD5 hash of the user's email address to the Gravatar server to retrieve the avatar. The request includes the User-Agent of user's browser, which contains information about user's device and browser. For details, please refer to Gravatar's privacy policy. When using `internal` avatar provider or disabling user avatars, no third-party services are relied
3. Maps: When user uses the map feature in ezBookkeeping to view transaction locations, user's browser or ezBookkeeping server program will retrieves map data from the third-party provider. During this process, the User-Agent of user's browser is included, which contains information about user's device and browser, but does not include specific location or any ezBookkeeping user data. If your map provider is set to Google Maps, Baidu Map or Amap, ezBookkeeping uses the provider's official JavaScript SDK. While ezBookkeeping itself does not share any user information with these services, the map providers may collect data about user's browser, device and location. For details, please refer to the map provider's privacy policy
4. Large Language Model - AI Clipboard Text Recognition / AI Image Recognition / Import AI Text Content Recognition Result / Import AI Image Content Recognition Result: When user uses these features in ezBookkeeping, ezBookkeeping sends the pasted text or uploaded receipt image along with all the user's transaction category names, account names and transaction tag names to the LLM provider. By default, these features in ezBookkeeping are disabled, and ezBookkeeping recommends using a self-hosted LLM service. If you choose to use a third-party LLM provider, please be aware of potential privacy risks and take steps to minimize data exposure (e.g. by removing sensitive information from the text before pasting, removing sensitive information from receipt images before uploading and avoiding storing sensitive data in transaction category names, account names or transaction tag names)
