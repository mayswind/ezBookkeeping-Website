...
## How to modify prompts of AI clipboard text recognition / AI image recognition and other LLM (Large Language Model) requests
...
4. Large Language Model - AI Clipboard Text Recognition / AI Image Recognition: When user uses AI clipboard text recognition or AI image recognition feature in ezBookkeeping, ezBookkeeping sends the pasted text or uploaded receipt image along with all the user's transaction category names, account names and transaction tag names to the LLM provider. By default, these two features in ezBookkeeping are disabled, and ezBookkeeping recommends using a self-hosted LLM service. If you choose to use a third-party LLM provider, please be aware of potential privacy risks and take steps to minimize data exposure (e.g. by removing sensitive information from the text before pasting, removing sensitive information from receipt images before uploading and avoiding storing sensitive data in transaction category names, account names or transaction tag names)
