...
| `longDateFormat` | `LongDateFormat` in [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) | The default long date format, supports `YearMonthDay` (Year Month Day), `MonthDayYear` (Month Day Year) and `DayMonthYear` (Day Month Year) |
| `shortDateFormat` | `ShortDateFormat` in [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) | The default short date format, supports `YearMonthDay` (Year Month Day), `MonthDayYear` (Month Day Year) and `DayMonthYear` (Day Month Year) |
| `longTimeFormat` | `LongTimeFormat` in [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) | The default long time format, supports `HourMinuteSecond` (24-hour time format), `MeridiemIndicatorHourMinuteSecond` (12-hour time format, AM/PM before the time) and `HourMinuteSecondMeridiemIndicator` (12-hour time format, AM/PM after the time) |
| `shortTimeFormat` | `ShortTimeFormat` in [`/src/core/datetime.ts`](https://github.com/mayswind/ezbookkeeping/blob/main/src/core/datetime.ts) | The default short time format, supports `HourMinute` (24-hour time format), `MeridiemIndicatorHourMinute` (12-hour time format, AM/PM before the time) and `HourMinuteMeridiemIndicator` (12-hour time format, AM/PM after the time) |
